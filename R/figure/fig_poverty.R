toplot_w <- svy_2022[, .(`2022` = sum(Final_unlinked_weight_fctr)), 
                     keyby = .(value = poverty)]
toplot_w <- toplot_w[!value == 'Refused/No Answer']
toplot_w[, frac := `2022` / sum(`2022`)]
toplot_w <- toplot_w[poverty,on=c(value = "poverty")]

toplot_w$value = stringr::str_wrap(as.character(toplot_w$value),20)
...
fig <- plot_ly(data = toplot_w) %>%
  ...
  layout(
    xaxis = list(
      title=list(text='Poverty Level', 
                 font = list(size = 16), standoff = 10)),
    yaxis = list(
      title = "Percentage",
      tickformat = ".0%",
      tickvals = seq(0, 0.93, by = 0.1),
      tickmode = "array"
    ),
